...
export default function AdminDashboard() {
  ...
  const loadAll = async () => {
    try {
      const [pRes, sRes, oRes, stRes, bRes, dRes] = await Promise.all([
        fetch("/api/products"), fetch("/api/shops"), fetch("/api/orders"), fetch("/api/settings"), fetch("/api/banners"), fetch("/api/discounts"),
      ]);
      ...
      const dd = await dRes.json(); setDiscounts(Array.isArray(dd) ? dd : []);
  ...
  const addDiscount = async () => {
    ...
    if (!res.ok) { alert(data.error || "خطا در ثبت کد تخفیف"); return; }
