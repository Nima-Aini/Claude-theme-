...
export async function PUT(req: NextRequest) {
  ...
  if (!Number.isInteger(numericShopId) || !Number.isFinite(numericAmount) || numericAmount <= 0)
    return NextResponse.json({ error: "اطلاعات واریز نامعتبر است" }, { status: 400 });

  const shop = await db.select().from(shops).where(eq(shops.id, numericShopId)).then(r => r[0]);
  if (!shop) return NextResponse.json({ error: "فروشگاه پیدا نشد" }, { status: 404 });
  const available = (shop.totalEarnings || 0) - (shop.paidEarnings || 0);
  if (numericAmount > available) return NextResponse.json({ error: "مبلغ واریز از مانده پورسانت بیشتر است" }, { status: 400 });

  await db.insert(payouts).values({ shopId: numericShopId, amount: numericAmount, description: description || null });

  // Update shop paid earnings
  const totalPaid = (shop.paidEarnings || 0) + numericAmount;
  await db.update(shops).set({ paidEarnings: totalPaid }).where(eq(shops.id, numericShopId));
  await sendPayoutSMS(shop.phone, numericAmount, totalPaid);
  await db.update(payoutRequests).set({ status: "paid" }).where(eq(payoutRequests.shopId, numericShopId));
