...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    await client.query(`
      ...
      CREATE TABLE IF NOT EXISTS shops (id SERIAL PRIMARY KEY, name VARCHAR(255) NOT NULL, slug VARCHAR(255) NOT NULL UNIQUE, image TEXT, banner_image TEXT, username VARCHAR(255) NOT NULL UNIQUE, password VARCHAR(255) NOT NULL, commission_rate INTEGER DEFAULT 10, total_earnings INTEGER DEFAULT 0, paid_earnings INTEGER DEFAULT 0, created_at TIMESTAMP DEFAULT NOW());
      CREATE TABLE IF NOT EXISTS customers (id SERIAL PRIMARY KEY, phone VARCHAR(20) NOT NULL UNIQUE, name VARCHAR(255), address TEXT, created_at TIMESTAMP DEFAULT NOW());
      CREATE TABLE IF NOT EXISTS orders (id SERIAL PRIMARY KEY, customer_id INTEGER NOT NULL, shop_id INTEGER NOT NULL, customer_name VARCHAR(255) NOT NULL, customer_phone VARCHAR(20) NOT NULL, customer_address TEXT NOT NULL, shipping_method VARCHAR(50) NOT NULL, total_amount INTEGER NOT NULL, commission_amount INTEGER DEFAULT 0, status VARCHAR(50) DEFAULT 'pending', tracking_link TEXT, items JSONB NOT NULL, created_at TIMESTAMP DEFAULT NOW());
      ...
      CREATE TABLE IF NOT EXISTS discount_codes (id SERIAL PRIMARY KEY, code VARCHAR(50) NOT NULL UNIQUE, percentage INTEGER NOT NULL, is_active BOOLEAN DEFAULT true, created_at TIMESTAMP DEFAULT NOW());
    ...
    return NextResponse.json({ success: true, message: "Database setup complete!", admin: "admin / admin123", shops: "sara/shop123, mahsa/shop456, rose/shop789" });
