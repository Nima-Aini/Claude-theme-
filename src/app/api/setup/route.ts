...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    // Ensure the requested management account exists on both fresh and existing databases.
    // Older databases may still contain the original `admin` account; migrate it safely.
    const adminPass = await bcryptjs.hash("Akma!2026#Nima@Secure", 12);
    const newAdmin = await client.query("SELECT id FROM admins WHERE username = $1 LIMIT 1", ["adminakma"]);
    if (newAdmin.rows.length === 0) {
      const oldAdmin = await client.query("SELECT id FROM admins WHERE username = $1 LIMIT 1", ["admin"]);
      if (oldAdmin.rows.length > 0) {
        await client.query("UPDATE admins SET username = $1, password = $2 WHERE id = $3", ["adminakma", adminPass, oldAdmin.rows[0].id]);
      } else {
        await client.query("INSERT INTO admins (username, password) VALUES ($1, $2)", ["adminakma", adminPass]);
      }
    } else {
      await client.query("UPDATE admins SET password = $1 WHERE username = $2", [adminPass, "adminakma"]);
    }

    const adminCheck = await client.query("SELECT id FROM admins WHERE username = 'adminakma'");
