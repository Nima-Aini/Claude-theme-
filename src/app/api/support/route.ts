...
export async function POST(req: NextRequest) {
  ...
  const msg = await db.insert(supportMessages).values({ ticketId: ticket[0].id, senderType: String(payload.type), message }).returning();
...
export async function PUT(req: NextRequest) {
  ...
  const msg = await db.insert(supportMessages).values({ ticketId, senderType: String(payload.type), message }).returning();
