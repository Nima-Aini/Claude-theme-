...
export default async function TrackPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const order = await db.select({ id: orders.id, status: orders.status, paymentStatus: orders.paymentStatus, createdAt: orders.createdAt, totalAmount: orders.totalAmount }).from(orders).where(eq(orders.id, Number(id))).then(r => r[0]);
  if (!order) notFound();
  const labels: Record<string,string> = { pending_payment: "در انتظار پرداخت", payment_failed: "پرداخت ناموفق", payment_cancelled: "پرداخت لغو شد", pending: "در انتظار بررسی", processing: "در حال پردازش", shipped: "ارسال شده", delivered: "تحویل شده" };
