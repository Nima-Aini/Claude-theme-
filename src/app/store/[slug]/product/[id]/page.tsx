...
export default async function ProductPage({ params }: { params: Promise<{ slug: string; id: string }> }) {
  ...
  const bestsellers = await db.select().from(products).where(eq(products.isBestseller, true)).limit(12);
  if (!shop || !product) notFound();
  const normalizedProduct = { ...product, isBestseller: product.isBestseller ?? undefined };
  const normalizedBestsellers = bestsellers
    .filter((p) => p.id !== product.id)
    .map((p) => ({ ...p, isBestseller: p.isBestseller ?? undefined }));
  return <ProductClient shop={shop} product={normalizedProduct} bestsellers={normalizedBestsellers} />;
}
