...
export default function StoreClient({ shop, products, bestsellers, sliderBanners, bottomBanners, settings, bestsellerTitle }: Props) {
  ...
  const discountedTotal = Math.floor(cartTotal * (1 - discountPercent / 100));

  const applyDiscount = async () => {
    if (!discountCode) return;
    const res = await fetch("/api/discounts/verify", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ code: discountCode }), credentials: "include" });
    const data = await res.json();
    if (data.valid) { setDiscountPercent(data.percentage); setDiscountMessage(`تخفیف ${data.percentage}% اعمال شد`); }
    else { setDiscountMessage("کد تخفیف نامعتبر است"); setDiscountPercent(0); }
  };

  const placeOrder = async () => {
    if (!checkoutForm.name || !checkoutForm.address) return;
    setLoading(true);
    try {
      ...
      if (res.ok) { setCart([]); setShowCheckout(false); setShowCart(false); setOrderSuccess(true); setDiscountCode(""); setDiscountPercent(0); setTimeout(() => setOrderSuccess(false), 5000); }
          ...
          {bestsellers.length > 0 && (
            <div className="px-4 pt-6 pb-2">
              ...
              <div ref={bestsellerRef} className="flex gap-3 overflow-x-auto no-scrollbar pb-2">
                {bestsellers.map((p) => (
                  <div key={p.id} className="flex-shrink-0 w-40 bg-white rounded-2xl overflow-hidden shadow-sm card-hover">
                    ...
                    <div className="p-3">
                      ...
                      <button onClick={() => addToCart(p)} className="w-full mt-2 py-1.5 rounded-lg text-white text-[11px] font-bold transition-all hover:opacity-90" style={{ background: primary }}>افزودن به سبد</button>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {cart.length === 0 ? (
            ...
            <div className="p-4">
              ...
              <div className="mt-5 rounded-2xl p-5 text-white" style={{ background: `linear-gradient(135deg, ${secondary}, ${primary})` }}>
                ...
                {discountPercent > 0 && (<div className="flex justify-between text-sm mb-2"><span className="text-yellow-300/80">تخفیف {discountPercent}%</span><span className="font-bold text-yellow-300">-{formatPrice(cartTotal - discountedTotal)}</span></div>)}
        ...
        <div className="animate-fadeIn">
          ...
          <div className="p-4 space-y-3">
            ...
            <div className="rounded-2xl p-5 text-white" style={{ background: `linear-gradient(135deg, ${secondary}, ${primary})` }}>
              ...
              <button onClick={placeOrder} disabled={loading || !checkoutForm.name || !checkoutForm.address} className="w-full mt-4 py-3 rounded-xl bg-white font-bold text-sm disabled:opacity-50" style={{ color: primary }}>{loading ? "در حال ثبت..." : "ثبت سفارش"}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="animate-fadeIn p-4">
          ...
          <div className="space-y-2.5">
            <div className="bg-white rounded-2xl p-5">
              <div className="flex items-center gap-3 mb-2"><div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: `${primary}10`, color: primary }}>{Icons.tag}</div><h3 className="font-bold text-sm" style={{ color: secondary }}>کد تخفیف</h3></div>
              <p className="text-gray-400 text-xs leading-relaxed">کد تخفیف را هنگام خرید در بخش سبد خرید وارد کنید.</p>
            </div>
            <div className="bg-white rounded-2xl p-5">
      ...
      <div className="fixed bottom-0 right-0 left-0 bg-white/80 backdrop-blur-xl border-t border-gray-100 z-30">
        <div className="flex justify-around items-center py-1.5 pb-safe max-w-lg mx-auto">
          {([
            ...
            <button key={tab.key} onClick={() => { setActiveTab(tab.key); setShowCart(false); setShowCheckout(false); if (tab.key === "orders") loadOrders(); }} className="flex flex-col items-center py-1.5 px-4 rounded-xl transition-all duration-200 relative" style={{ color: activeTab === tab.key ? primary : "#c0c0c0" }}>
